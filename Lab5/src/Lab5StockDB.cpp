...
Node* StockDB::getHead(){
	return head;
}

Node* StockDB::getTail(){
	return tail;
}

void StockDB::setHead(Node* h){
	head = h;
}

void StockDB::setTail(Node* t){
	tail = t;
}
...
void StockDB::reverseRecursionPrint(Node* current){
	if(current != NULL){
		reverseRecursionPrint(current->getNext());
